...
<template>
    <div class="buttons-box">
      ...
      <div class="round">
        <h2>圆角按钮</h2>
        <div class="buttons">
          <m-button v-for="button in defaultButtons" :key="button.type" :type="button.type" round>{{ button.label }}</m-button>
        </div>
      </div>
      <div class="link">
        <h2>链接按钮</h2>
        <div class="buttons">
          <m-button v-for="button in defaultButtons" :key="button.type" :type="button.type" link>{{ button.label }}</m-button>
        </div>
      </div>
      <div class="text">
        <h2>文字按钮</h2>
        <div class="buttons">
          <m-button v-for="button in defaultButtons" :key="button.type" :type="button.type" text>{{ button.label }}</m-button>
        </div>
      </div>
